...
namespace videoSitecore2.Controllers
{
    public class VideosController : Controller
    {
     ...
     /*   public ActionResult IndexViewData(int? id, string searchString, int? pageNumber, int metaDataType)
...

        {
            ...
            var result = (from vi in _context.Videos
                          ...
                          select new Videoview
                          {
                              ...
                              Title = ti.Title,
                              metadataTitleId= me.MetadataTitleId,
                              metaDataTypeId=ti.MetaDataTypeId
                          }).Distinct().ToList();
        ...
        public ActionResult startTimeEventSegmenTtype(int? id, int? metadataTitleId)

        {
            ...
            List<Localisation2Seg> local2 = (from vi in _context.Videos
                                          join me in _context.MetaData on vi.Id equals me.VideoId
                                          join ti in _context.MetaDataTitles on me.MetadataTitleId equals ti.Id
                                          where vi.Id.Equals(id) && ti.Id.Equals(metadataTitleId)
                                          select new Localisation2Seg
                                          {
                                              tcin = me.StartTime,
                                              tcout = me.EndTime,
                                              tclevel = 1,
            ...
            SublocalisationsSeg Sub = new SublocalisationsSeg();
            Sub.localisation = local2;



            List<LocalisationSeg> local = (from vi in _context.Videos
                                        where vi.Id.Equals(id)
                                        select new LocalisationSeg
                                        {
                                            type = "events",
                                            tcin = "00:00:00.0000",
                                            tcout = "00:00:15.0000",
            ...
            RootjsonclassSeg rootjsonclass = new RootjsonclassSeg();
            rootjsonclass.localisation = (List<LocalisationSeg>)local;
            // rootjsonclass.id = "events-amalia01";
            rootjsonclass.id = metadataTitleId.ToString();
            rootjsonclass.type = "segments";
            rootjsonclass.algorithm = "demo-video-generator";
            rootjsonclass.processor = "Ina Research Department - N. HERVE";
            rootjsonclass.processed = 1421141589286;
            rootjsonclass.version = 1;
        ...
        public JsonResult getTitleList(int? id )//, int? metaDataTypeId)

        {
            var TypeList = (from me in _context.MetaData
                             join ti in _context.MetaDataTitles on me.MetadataTitleId equals ti.Id
                             join ty in _context.MetaDataTypes on ti.MetaDataTypeId equals ty.Id
                             where me.VideoId.Equals(id) 
                             select new
                             {
                                 Id=ty.Id,
            ...
            var TitleList=new List<List<string>>();
            int k = 0;
            foreach (var item in TypeList)
            {
               var i = (from me in _context.MetaData
                                 join ti in _context.MetaDataTitles on me.MetadataTitleId equals ti.Id
                                 where me.VideoId.Equals(id) && ti.MetaDataTypeId.Equals(item.Id)
                                 select new 
                                 {
                                     Title = ti.Title,
                                 }).Distinct().ToList();
                TitleList.Add( new List<string>());
                foreach (var j in i)
                {
                    TitleList[k].Add(j.Title);
                }
                k++;
            }
            ...
            return Json(new {TypeList, TitleList });
    ...
    public class SublocalisationsSeg
    {
        public List<Localisation2Seg> localisation { get; set; }
    ...
    public class LocalisationSeg
    {
        public SublocalisationsSeg sublocalisations { get; set; }
        public string type { get; set; }
        public string tcin { get; set; }
        public string tcout { get; set; }
        public int tclevel { get; set; }
    ...
    public class RootjsonclassSeg
    {
        public List<LocalisationSeg> localisation { get; set; }
        public string id { get; set; }
        public string type { get; set; }
        public string algorithm { get; set; }
        public string processor { get; set; }
